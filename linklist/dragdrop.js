...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    window.addCarriage = function() {
      ...
      updateCarriageCount(); // เรียกใช้งานเมื่อมีการเพิ่มโบกี้
    };
...
window.removeAllCarriages = function () {
  const train = document.getElementById("train");
  train.innerHTML = ""; // ลบเนื้อหาทั้งหมดของคอนเทนเนอร์รถไฟ
  carriageCount = 0;
  updateCarriageCount(); // เรียกใช้งานเพื่ออัปเดตจำนวนโบกี้
};

function updateCarriageCount() {
  document.getElementById("carriageCountDisplay").textContent =
    "จำนวนโบกี้: " + carriageCount;
}
...
window.addCarriage = function () {
  if (carriageCount < 15) {
    ...
    alert("ไม่สามารถเพิ่มโบกี้ได้ เกินจำนวนที่กำหนด");
  }
};
});
